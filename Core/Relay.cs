    ...
    public class Relay {
            ...
            if (@event.FromBuffer(buffer)) {
                ...
                if (LastHandshake.Protocol != ProtocolVersion)
                    Logger.LogWarning(
                        ...
                        tag: nameof(HandleHandshake));
                if (LastHandshake.MaxPacketSize > 0)
                    Connector.Mtu = LastHandshake.MaxPacketSize;
            }
            else
                Logger.LogWarning($"Failed to parse {nameof(Types.Handshakes.HandshakeResponse)} from buffer",
                    tag: nameof(HandleHandshake));
        }
        ...
        private void HandleLatency(ushort state, Buffer buffer) {
            ...
            if (@event.FromBuffer(buffer)) {
                ...
                Logger.LogWarning($"Failed to parse {nameof(Types.Latency.LatencyResponse)} from buffer",
                    tag: nameof(HandleLatency));
        }
        ...
        private void HandleInstance(ushort state, ResponseType type, Buffer buffer) {
            ...
            if (instance == null) {
                Logger.LogWarning($"Received packet for unknown instance ID {iid} (type: {type}, state: {state})",
                    tag: nameof(HandleInstance));
                return;
            }
        ...
        private void HandleDisconnect(ushort state, Buffer buffer) {
            ...
            if (@event.FromBuffer(buffer)) {
                ...
                Logger.LogWarning($"Failed to parse {nameof(Types.Disconnect.DisconnectEvent)} from buffer",
                    tag: nameof(HandleDisconnect));
        }
        ...
        public async UniTask<T> Request<T>(
            ...
            if (!result.Success) {
                ...
                Logger.LogWarning($"Failed to emit {result.State}:{@out}", tag: nameof(Request));
                return null;
            }
            ...
            Logger.LogWarning($"{stateContainer[0]}:{@out} timeout", tag: nameof(Request));
            return null;
            ...
            void OnPacket(ushort s, ResponseType t, Buffer payload) {
                ...
                else Logger.LogError($"Failed to parse {expectedState}:{@in} to {typeof(T)}", tag: nameof(Request));
            }
        }
        ...
        public async UniTask<Types.Latency.LatencyResponse> Latency() {
            ...
            if (res == null || res.Challenge != req.Challenge) {
                if (res != null)
                    Logger.LogWarning($"Latency response challenge mismatch: {res.Challenge} != {req.Challenge}",
                        tag: nameof(Latency));
            }
