...
namespace Nox.Relay.Core.Rooms {
	...
	public class Room {
		...
		/// </summary>
		public UnityEvent<TransformEvent> OnTransform { get; } = new();

		/// <summary>
		/// Event invoked when a custom event is received in the room.
		/// </summary>
		public UnityEvent<EventEvent> OnEvent { get; } = new();

		/// <summary>
		/// Event invoked when a properties event is received in the room.
		/// </summary>
		public UnityEvent<PropertiesEvent> OnProperties { get; } = new();

		/// <summary>
		/// Event invoked when an avatar changed event is received in the room.
		/// </summary>
		...
		private void HandleAvatarChanged(ushort state, Buffer buffer) {
			var avatarChanged = new AvatarChangedEvent { State = state, Connection = Connection, Room = this };
			if (!avatarChanged.FromBuffer(buffer)) {
				Logger.LogWarning("Failed to parse AvatarChangedEvent", tag: Tag);
				return;
			}

			OnAvatarChanged.Invoke(avatarChanged);
		}
	...
	private void HandleEnter(ushort state, Buffer buffer) {
		...
		// Update room properties with server values
		Tps = enter.Tps;
		Threshold = enter.Threshold;
		RenderEntity = enter.RenderEntity;
