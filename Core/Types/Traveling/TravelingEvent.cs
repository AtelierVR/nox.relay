	...
	public class TravelingEvent : RoomResponse {
		...
		public override bool FromBuffer(Buffer buffer) {
			...
			if (UseUrl) {
				...
				Identifier = new Identifier(WorldIdentifierExtensions.WorldType, buffer.ReadUInt(), meta, buffer.ReadString());

				var version = buffer.ReadUShort();
				if (version != WorldIdentifierExtensions.DefaultVersion)
					meta.Add(WorldIdentifierExtensions.VersionQuery, new[] { version.ToString() });

				if (UseHash) {
					Hash = buffer.ReadBytes(32);
					meta.Add(WorldIdentifierExtensions.HashQuery, new[] { Hash.ToBase64() });
				}

				if (HasPassword) {
					Password = buffer.ReadBytes(buffer.ReadUShort());
					meta.Add(WorldIdentifierExtensions.PasswordQuery, new[] { Password.ToBase64() });
				}

				Identifier = new Identifier(Identifier.Type, Identifier.Id, meta, Identifier.Server);
			} else if (IsReady) {
		...
		public override string ToString()
			=> $"{GetType().Name}[Results={Results}"
				...
				+ (UseNode ? $", World={Identifier.ToString()}" : "")
				+ "]";
	}
}